...
export default {
  ...
  computed: {
    ...
    slideStyle() {
      let styles = {
        width: px2rem(this.parent.slideWidth),
      }
      if (this.isCurrent) {
        const margin = this.parent.shrink;
        styles.width = px2rem(this.parent.slideWidth - margin * 2);
        styles.transform = `translateX(${px2rem(margin)})`;
      } else {
      ...
      return Object.assign(styles, {
        'border-width': px2rem(this.parent.border),

        'height': px2rem(this.parent.slideHeight),
  ...
  methods: {
    ...
    calculatePosition(i, positive, zIndex) {
      ...
      const transform = (positive)
          ? 'translateX(' + (px2rem(leftRemain)) + ') translateZ(-' + px2rem(z) + ') ' +
          'rotateY(-' + y + 'deg)'
          : 'translateX(-' + (px2rem(leftRemain)) + ') translateZ(-' + px2rem(z) + ') ' +
          'rotateY(' + y + 'deg)'
