<template>
  <div class="carousel-3d-container" :style="{height: px2rem(this.slideHeight)}">
    <div class="carousel-3d-slider" :style="{width: px2rem(this.slideWidth), height: px2rem(this.slideHeight)}">
...
export default {
  ...
  methods: {
    ...
    getSlideCount() {
      ...
      } else if (typeof this.$slots.default === 'function') {
        slots = this.$slots.default()
      }
      if (slots.length) {
        return slots.length
    ...
    setSize() {
      this.$el.style.cssText += 'height:' + px2rem(this.slideHeight) + ';'
      this.$el.childNodes[0].style.cssText += 'width:' + px2rem(this.slideWidth) + ';' + ' height:' + px2rem(this.slideHeight) + ';'
    }
  },
